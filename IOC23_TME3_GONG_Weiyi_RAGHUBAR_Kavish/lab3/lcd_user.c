...
// Afin de dire au compilateur de ne pas optimiser afin d'éviter les erreurs 
struct gpio_s {
    uint32_t gpfsel[7];
    uint32_t gpset[3];
    uint32_t gpclr[3];
...
void lcd_message(const char *txt,int x, int y)
{
    ...
    for (i = 0, l = 0; (l < 4) && (i < strlen(txt)); ++l) {
        if(y==0){
            lcd_command(LCD_SETDDRAMADDR + a[l]+x);
            for (; (i < (l + 1) * len) && (i < strlen(txt)); ++i) {
                lcd_data(txt[i]);
            }
        }
        if(y==1){
            lcd_command(LCD_SETDDRAMADDR + a[l]+x+40);
            for (; (i < (l + 1) * len) && (i < strlen(txt)); ++i) {
                lcd_data(txt[i]);
            }
        }
        if(y==2){
            lcd_command(LCD_SETDDRAMADDR + a[l]+x+20);
            for (; (i < (l + 1) * len) && (i < strlen(txt)); ++i) {
                lcd_data(txt[i]);
            }
        }
        if(y==3){
            lcd_command(LCD_SETDDRAMADDR + a[l]+x+60);
            for (; (i < (l + 1) * len) && (i < strlen(txt)); ++i) {
                lcd_data(txt[i]);
            }
        }
...
{
    //cette fonction nous permet de déplacer le curseur à la position x,y sur l'afficheur
    //la coordonnée 0,0 correspond à l'adresse 00
    long tab[19][3];
    int i;
    for (i=0; i<20; i++)
    {
    ...
    int cursor_adr;
    cursor_adr = tab[x][y];
    //int lcd_mask = LCD_DISPLAYCONTROL | LCD_DC_DISPLAYON | LCD_DC_CURSORON | LCD_DC_BLINKON;
    int j=0;
...
int main(int argc, char **argv)
{
    ...
    // lcd_message(argv[1]);
    // lcd_set_cursor(0,3);
    // lcd_command(LCD_CURSORSHIFT|LCD_DISPLAYCONTROL | LCD_DC_DISPLAYON |LCD_CS_DISPLAYMOVE);
   
    //lcd_set_cursor(0,3);
    lcd_command(LCD_DC_CURSORON|LCD_DISPLAYCONTROL | LCD_DC_DISPLAYON |LCD_CS_DISPLAYMOVE);
    //lcd_message(argv[1]);
    //lcd_message(argv[1]);
    // int y;
    // for(y=0;y<4;y++)
    // {
    //     lcd_message(argv[1],0,y);
    //     sleep(1);
    // }
    //lignes(argv[1]);
    lcd_message(argv[1],0,0);
