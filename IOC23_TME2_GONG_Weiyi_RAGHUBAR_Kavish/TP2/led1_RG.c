...
static void gpio_write(int pin, bool val)//fonction de gpio write, comme on a fait avant
{
    if (val)
...
static ssize_t 
write_led_XY(struct file *file, const char *buf, size_t count, loff_t *ppos) {//Fonction pour ecrire les valeur dans gpio en led
    printk(KERN_DEBUG "write()\n");
    int val;
    if(buf=='0')val=0;//Comparer si buf est 0, on ecrit 0
    if(buf=='1')val=1;//Si buf est 1, on ecrit 1
    gpio_write(LED1,val);//Ecrit les valeur dans led
    return count;
}
...
module_init(mon_module_init);//Chaque fois on lance le driver, il a fait le fonction
module_exit(mon_module_cleanup);//Chaque fois on ferme le driver
